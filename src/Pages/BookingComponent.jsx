...
import HomeFilter from "../components/HomeFilter"; // عدّل المسار حسب مكان الملف عندك


const UNIFIED_SPECIALTIES = [
  "قلب", "مسالك بولية", "أطفال", "جلدية", "عظام", "أسنان",
  "عيون", "مخ وأعصاب", "باطنة", "التخدير", "جراحة عامة", "نساء وتوليد"
];
...
const BookingComponent = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      let q;
      if (activeTab === 'call') {
        q = query(collection(db, "Doctors"), where("specialty", "==", specialty.trim()));
      } else {
        q = query(collection(db, "Doctors")); // تعديل لاحق ممكن لإضافة فلاتر أخرى
      }
  ...
  return (
    <div className="relative w-full min-h-screen flex justify-center items-center overflow-hidden bg-gray-100">
      ...
      >
        ...
        <form onSubmit={activeTab === 'call' ? handleSearch : (e) => e.preventDefault()} className="space-y-6">
          {error && (
          ...
          {activeTab === 'call' && <SearchButton loading={loading} />}
        </form>
